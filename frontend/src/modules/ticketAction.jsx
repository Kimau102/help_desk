...
export function TicketAction({ ticketInfo }) {
	...
	const StatusSelectOptionChip = () => {
		...
		return (
			<>
				...
				>
					<MenuItem onClick={() => handleStatusSelect('Open')}>
						Open
					</MenuItem>
					<MenuItem onClick={() => handleStatusSelect('Processing')}>
						Processing
					</MenuItem>
					<MenuItem onClick={() => handleStatusSelect('Solve')}>
						Solve
					</MenuItem>
					<MenuItem onClick={() => handleStatusSelect('On Hold')}>
						On Hold
					</MenuItem>
					<MenuItem onClick={() => handleStatusSelect('Closed')}>
						Closed
					</MenuItem>
	...
	const PrioritySelectOptionChip = () => {
		const [anchorEl, setAnchorEl] = React.useState(null)
		const open = Boolean(anchorEl)
		const handleClick = (event) => {
			setAnchorEl(event.currentTarget)
		...
		return (
			<>
				<Stack direction='row' spacing={1}>
					<Chip
						label={selectedPriority || ticketInfo.priority}
						onClick={handleClick}
						Icon={<ArrowDropDownIcon />}
						variant='outlined'
					/>
				...
				>
					<MenuItem onClick={() => handlePrioritySelect('Ugent')}>
						Ugent
					</MenuItem>
	...
	const handleClickClose = () => {
		...
		setSelectedPriority(ticketInfo.priority)
	}
			...
			>
				...
				<DialogContent dividers={true}>
					...
					<Typography
						...
						style={{ display: 'flex', alignItems: 'center' }}
					>
						Priority: <PrioritySelectOptionChip />
					</Typography>
